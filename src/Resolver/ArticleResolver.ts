...
export class ArticleResolver {
  @Query(returns => Article, {
    nullable: true,
    description:
      "Get an Article by its ID or SLUG. One of those 2 must be provided for the endpoint to perform a search in the DB.",
  })
  article(
  ...
  @Query(returns => [Article], {
    description:
      "Generic endpoint to query the Articlres. Requires pagination and provides sort and filter options.",
  })
  async articles(
  ...
  author(@Root() article: Article, @Ctx() ctx: RequestContext) {
  ...
  ledger(@Root() article: Article, @Ctx() ctx: RequestContext) {
  ...
  @FieldResolver(returns => MediaImage, {
    description:
      "The media entity associated with the thumbnail of the article, provides additional informations on the thumbnail such as resolution, base64 placeholder and mime type.",
    nullable: true,
  })
  thumbnailMedia(@Root() article: Article, @Ctx() ctx: RequestContext) {
  ...
  revisions(@Root() article: Article, @Ctx() ctx: RequestContext) {
  ...
  royaltiesSplits(@Root() article: Article, @Ctx() ctx: RequestContext) {
    return ctx.articlesRoyaltiesSplitsLoader.load(article.id)
  }

  @FieldResolver(returns => [Action], {
    description:
      "A list of all the actions related to the Article. **Not optimized to be run on multiple generative tokens at once, please use carefully*.",
  })
  actions(
    @Root() article: Article,
  ...
  async relatedArticles(
    ...
    /**
     * hard limit of 512 characters for the search query, so we shuffle the tags
     * and add them to the query until we reach the limit
     */
    const shuffledTags = shuffle(article.tags)
    const searchQuery_eq = shuffledTags.reduce(
  ...
  @FieldResolver(returns => String, {
    nullable: true,
    description:
      "If any, returns the moderation reason associated with the Article",
  })
  async moderationReason(@Root() article: Article, @Ctx() ctx: RequestContext) {
