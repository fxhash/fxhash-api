...
export class ListingResolver {
  @FieldResolver(returns => User, {
    nullable: true,
    description:
      "The user who made the listing. *If the listing was accepted/cancelled, this may not be equal to the gentk owner*",
  })
  issuer(@Root() listing: Listing, @Ctx() ctx: RequestContext) {
  ...
  objkt(@Root() listing: Listing, @Ctx() ctx: RequestContext) {
  ...
  article(@Root() listing: Listing, @Ctx() ctx: RequestContext) {
  ...
  @Query(returns => [Listing], {
    description:
      "The go-to endpoint to explore Listings on the marketplace. This endpoint both returns Listings made on the old and new marketplace contracts. **By default, only returns active listings (not accepted nor cancelled)**",
  })
  async listings(
    ...
    if (filters?.searchQuery_eq) {
      const searchResults = await searchIndexMarketplace.search(
      ...
      // otherwise we filter the results by their (id, version) pair
        ...
        query.where(`(listing.id, listing.version) IN(${formatted})`)
      }
    ...
    // custom filters
    if (
      filters?.fullyMinted_eq != null ||
      filters?.authorVerified_eq != null ||
      filters?.tokenSupply_lte != null ||
      filters?.tokenSupply_gte != null
    ) {
      ...
      if (filters?.tokenSupply_lte != null) {
        query.andWhere("token.supply <= :sizeLte", {
          sizeLte: filters.tokenSupply_lte,
        })
      }
      if (filters?.tokenSupply_gte != null) {
        query.andWhere("token.supply >= :sizeGte", {
          sizeGte: filters.tokenSupply_gte,
        })
      }
    }
  ...
  @Query(returns => [Listing], {
    nullable: true,
    description:
      "Given a list of Listing identifiers (ID + contract-version), outputs the associated listings.",
  })
  async listingsByIds(
    ...
    const query = Listing.createQueryBuilder("listing")
      .select()
      .where(
        `(listing.id, listing.version) IN(${ids.map(
          id => `(${id.id},${id.version})`
        )})`
      )
