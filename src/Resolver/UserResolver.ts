...
export class UserResolver {
  @FieldResolver(returns => [UserAuthorization], {
    description:
      "Users can have granular moderation rights to certain features on the fxhash contracts. This field outputs a list of those authorizations in a human readable way.",
  })
  ...
  @FieldResolver(returns => [Objkt], {
    description:
      "The gentks owned by the user. Can be used to explore their collection using various filters.",
  })
  async objkts(
    @Root() user: User,
    ...
    if (!sort || Object.keys(sort).length === 0) {
      sort = {
        // order by creation date as IDs will start from 0 after V3
        createdAt: "DESC",
      }
    }
  ...
  @FieldResolver(returns => MediaImage, {
    description:
      "The media entity associated with the user avatar, provides additional informations on the avatar such as resolution, base64 placeholder and media type.",
    nullable: true,
  })
  avatarMedia(@Root() user: User, @Ctx() ctx: RequestContext) {
  ...
  async generativeTokensFromObjktFilters(
    ...
    // FILTER / SORT
    query = await objktQueryFilter(query, {
      general: filters,
    })
  ...
  @FieldResolver(returns => [GenerativeToken], {
    description:
      "Returns the tokens authored by the user, either directly or as a collaboration through a collaboration contract.",
  })
  ...
  articlesOwned(@Root() user: User, @Ctx() ctx: RequestContext) {
    return ctx.usersArticleLedgersLoader.load(user.id)
  }

  @FieldResolver(returns => [Listing], {
    description:
      "The Listings created by the user. By default, this endpoint only returns the active Listings (not cancelled not accepted). **This endpoint should only be used on a single user at once as it has not been optimized for batch yet**.",
  })
  listings(
    @Root() user: User,
    @Ctx() ctx: RequestContext,
  ...
  @FieldResolver(returns => [User], {
    nullable: true,
    description:
      "If the user is a regular user account, returns its list of collaboration contracts created on the fxhash collaboration contract factory.",
  })
  async collaborationContracts(@Root() user: User, @Ctx() ctx: RequestContext) {
  ...
  @FieldResolver(returns => [User], {
    nullable: true,
    description:
      "If the user is a collaboration contract, this endpoint will return the list of collaborators on the contract.",
  })
  async collaborators(@Root() user: User, @Ctx() ctx: RequestContext) {
  ...
  @FieldResolver(returns => [Action], {
    description:
      "The actions in which the user is either the issuer or the target. Paginated, can be filtered and sorted.",
  })
  async actions(
    @Root() user: User,
    @Ctx() ctx: RequestContext,
  ...
  @FieldResolver(returns => [Action], {
    description:
      "The secondary market sales in which the user is involved as a recipient (either as the direct seller or as an artist)",
  })
  async sales(
    @Root() user: User,
    @Ctx() ctx: RequestContext,
  ...
  @FieldResolver(returns => String, {
    nullable: true,
    description:
      "If any, returns the moderation reason associated with the user",
  })
  async moderationReason(@Root() user: User, @Ctx() ctx: RequestContext) {
  ...
  @Query(returns => User, {
    nullable: true,
    description:
      "Find a user by it's ID (tezos address) or by it's name as they set it on the fxhash user register.",
  })
  async user(
